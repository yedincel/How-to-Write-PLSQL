...
BEGIN
  ...
  if v_sonuc then
    dbms_output.put_line(v_cumle || ' cümlesi içinde ' || v_aranan_kelime || ' kelimesi geçmektedir.');
  else
    dbms_output.put_line(v_cumle || ' cümlesi içinde ' || v_aranan_kelime || ' kelimesi geçmemektedir!');
  end if;
END;
